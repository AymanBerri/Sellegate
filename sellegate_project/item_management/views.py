...
def UserInventortyAPIView(request, user_id):
    # Retrieve user's inventory
    inventory = Item.objects.filter(seller_id=user_id)
    
    # Serialize inventory data
    inventory_data = [{'item_id': item.id, 'title': item.title, 'status': item.status} for item in inventory]
    
    # Return inventory data as response
